...
The badge is **binary** — earned or not. It will be recorded on-chain in the `TrustRegistry` contract and queryable via public API. The certification platform is currently in development.
...
The operational platform behind the CTS — the TrustRegistry smart contract, the trust badge issuance system, and the certification infrastructure — is operated by **CashFlowIntel**, the registered entity that runs and enforces the standard. The framework is open; the platform is the business.
